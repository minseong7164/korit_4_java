...
//        List<Map<String, String>> entryList2 = strMap1.entrySet();      // Map -> List 대입이 불가능하다는 걸 보여주는 예시
        ...
        // key Set 출력
        Set<String> keySet = strMap1.keySet();      // 그러면
        System.out.println(keySet);

        // value Set 출력
//        Set<String> valueSet = strMap1.values();  // 오류 발생 예시 -> 필요한 타입 Set / 제곱된 타입 Collection
        // 이상에서 문제가 발생하는 이유 key는 어차피 중복을 허용하지 않기 때문에 Set으로 형변환이 가능한 반면
        // 복수의 key에서 동일한 value가 존재할 수 있기 때문에 Set으로 생성하는 것이 불가능합니다.
        Collection<String> values = strMap1.values();
        System.out.println(values);                     // 그 와중에 .sort() 할 때 처럼 Collection가 아니라 Collection입니다.
    }
}
